...
################################################################
# Binary Text Feature Extraction
...
###########################################################
#The picture is divided into m*n blocks
def cut_image(image, m, n):
    ...
    for i in range(0,n):#Double loop, generate m*n pictures based on the position of the original picture
        for j in range(0,m):
...
def LBP_feature_extrector(roi_file):
    ...
    for i, image_roi in enumerate(images_roi):
        ...
        #img_roi_raw_lbp_cut = cut_image(img_roi_raw_lbp, 4, 4) #Divided into 4 x 4
        #Block Display
        #plt.figure(figsize = (30, 30))
...
def FLANN_based_Matcher(img1_path, img2_path):
    ...
    #Calculate the match score with two decimal places
...
#Draw a histogram of the intra-class and inter-class distances of the first group of samples
...
#Sample No. 2 Calculated Distribution Histogram
#cal_scores('FLANN', 2)
#No. 4 sample calculation histogram
#cal_scores('BIN', 4)
